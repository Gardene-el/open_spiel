...
enum class GoRule : uint8_t { kClassic = 0, kAuto = 1, kReverseAuto = 2,kNumRuleTypes};
...
class Neighbours8 {
  public:
   explicit Neighbours8(const VirtualPoint p);
 
   Neighbours8 &operator++();
   const VirtualPoint operator*() const;
   explicit operator bool() const;
...
class GoBoard {
 public:
  ...
  inline Action VirtualActionToAction(int virtual_action,GoRule go_rule) const {
    return go_cellular::VirtualActionToAction(virtual_action, board_size_,go_rule);
  }
  inline int ActionToVirtualAction(Action action) const {
    return go_cellular::ActionToVirtualAction(action, board_size_);
  }
  inline GoRule ActionToRule(Action action) const {
    return go_cellular::ActionToRule(action, board_size_);
  }
  ...
  bool PlayMove(VirtualPoint p, GoColor c, GoRule r);

  //临时占位
  bool IsVirtualPointPass(VirtualPoint p) const {
    return p == kVirtualPass;
  }
 ...
 private:
  ...
  // 添加新的函数声明
  void ExecuteCellularAutomata();
  void ExecuteCellularReverseAutomata();
  void CheckAndRemoveStonesWithoutLiberties();
  void UpdateChainAfterStoneRemoval(VirtualPoint p);
  int CountNeighborsOfColor(VirtualPoint p, GoColor c) const;
